...
# Part One: Structuring Network Data

First, I clean the data and utilize a loop to create the nine sociomatrices by decade, keeping only the top 120 exporters. I store the matrices in a list called 'mats.'
...
# Part Two: Visualizing Network Data

Next I create graphs from the 9 sociomatrices. Edge sizes are based on volume of trade, while node sizes are based on eigenvector centrality. I use eigenvector centrality as countries' trade with other big traders seems a more important indicator of influence in the global trade market than simply trade connectedness (it also looked the best). Nodes are colored by continent.
...
width_1920 <- log(E(g_1920)$weight)/40
...
width_1930 <- E(g_1930)$weight/400
...
width_1940 <- log(E(g_1940)$weight)/60
...
width_1950 <- log(E(g_1950)$weight)/80
...
width_1960 <- log(E(g_1960)$weight)/80
...
width_1970 <- log(E(g_1970)$weight)/160
...
width_1980 <- log(E(g_1980)$weight)/300
...
width_1990 <- log(E(g_1990)$weight)/600
...
width_2000 <- log(E(g_2000)$weight)/700
...
And I place the plots in one figure with a legend of colors
...
legend('bottomright', legend= c('Americas', 'Asia', 'Europe', 'Oceania', 'Africa'), col = c('green', 'red', 'blue', 'yellow', 'purple'), pch=20, bty = 'n')
...
First I create vectors of network densities and reciprocity scores. I then use ggplot2 to show the changes over time.
...
I now calculate the node-level measures, and create a table of the top 30 countries for each.

```{r}
in_1920 <- sort(degree(g_1920), decreasing=TRUE)
in_2000 <- sort(degree(g_2000), decreasing=TRUE)
...
stargazer(top_actors[1:6], type='text', summary=FALSE)
stargazer(top_actors[7:12], type='text', summary=FALSE)
```
  
  The top 30 actors for the in/out/total degree measures seem to simply be the richest countries in the world. It makes sense these countries have the most trade. 
  The substnative explanation of the top closeness actors are less clear. These are countries whose total distance to other nodes are lowest. In other words, you do not have to go through many nodes to go from one of these to any other actor.
  The betweeness scores indicate countries that serve as bridges to other countries. They tend to trade with two groups of countries who do not trade with eachother. Many of these are middle-income countries, who likely trade with both poor and rich countries.
  Finally, the top eigenvector centrality nodes are very similar to the degree centrality top actors. These are the countries who trade alot with other countries who trade alot. Again, these are the rich countries.


And finally, I create a correlation matrix of the measures.

```{r}
measures_1920 <- as.data.frame(cbind(in_1920, out_1920, total_1920, clos_1920, bet_1920, eig_1920))

corr_1920 <- as.data.frame(cor(measures_1920))


measures_2000 <- as.data.frame(cbind(in_2000, out_2000, total_2000, clos_2000, bet_2000, eig_2000))

corr_2000 <- as.data.frame(cor(measures_2000))
...
# Part Four: Decomposing a matrix

First I recreate the paper visualization, with nodes colored by the averageId.node variable.
...
# add averageId.node attribute to each node 
for(i in V(g)$name) {
...
Next, I recreate the GLM model, and visualize error by actor
...
There does seem to be underlying structure missed by a GLM model, as error do not seem randomly distributed. NEgative residuals are close to zero, while there are extensive positive residuals. Additionally, the final two actors have mich higher average error than the rest of the dataset.
